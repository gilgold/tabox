...
const renderPanel = async (recovery) => {
    let result;
    await act(async () => {
        result = render(
...
test('hides the card when there are no orphans', async () => {
    await renderPanel(makeRecovery({ orphans: [], orphanCount: 0, showEntry: false }));
